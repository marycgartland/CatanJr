...
public class Board {
	...
	// This method determines which player has the most cocotiles. If 'G' is no longer at the centre
	// (design[8][18]), the user with the most cocotiles can add a lair there.
	// ---------------------------------------------------------------------------------------------------------
	public void mostCocotiles(ArrayList<Player> players) {
		...
		if (design[8][18] != 'G') { // Check that the Ghost captain isn't at the centre of the board
			for (int i = 0; i <= players.size() - 1; i++) {
				// If player has more cocotiles then another need to remove the previous players
				// lair and reduce the number
	...
	public void moveGhostCaptain() {
		...
		while (!integer_given) {
			...
			showIslandNumberLayout(ghostCaptain.getGhostCaptainLocation()); // Replace centers of island with their//
																			// island #'s
	...
	public void showIslandNumberLayout(int current_GC_location) {
		for (int i = 0; i <= 17 - 1; i++) {
			for (int j = 0; j <= 38 - 1; j++) {
				// Make sure user isn't trying to place ghost captain on top of someone's
				// existing lair on spooky island or ghost captain
	...
	// ---------------------------------------------------------------------------------------------------------
	// ---------- Method: placeShip ----------------------------------------------------------------------------
	// Place a users ship on the boar. Search for upper case of colour to search for lairs to connect ships to.
	// ---------------------------------------------------------------------------------------------------------
	public boolean placeShip(Player player) {
		...
		// if options are available, let user decide where they would like to build
		// their ship
		if (count > 0) {
